...
export class UtilsService {
	...
	redirect(on, params, navigate?) :any {
			...
			for(let key of options.keys){
				if(Array.isArray(item[key])){
					...
					for(let word of item[key]) {
						if (word.toString().normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase().indexOf(search) > -1) {
							items.push(item);
							result = true;
							break;
						}
				...
				if (item[key].toString().normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase().indexOf(search) > -1) {
					items.push(item);
					break;
				}
			}
